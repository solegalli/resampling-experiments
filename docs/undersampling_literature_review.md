...
| Kim & Hwang | 2022 | PLoS One | ROS, SMOTE, BorderlineSMOTE, RUS, CNN, NearMiss2, SMOTETomek; 31 datasets (IR 1.1–578) | AdaBoost, **XGBoost, RF**, SVM, LDA, Lasso, Ridge, ElasticNet | **YES** (without-sampling) | **AUROC, AUPRC** | n/a (AUC/PR) | **Yes (5×2cv paired t-test)** | Sampling significantly changed performance in only ~10–12% of cases, and **more often reduced** than improved it (78.7% of significant AUPRC changes, 61.3% of AUROC, were decreases); sampling was not needed for the optimal classifier on most of the 31 datasets. | [PMC9333262](https://pmc.ncbi.nlm.nih.gov/articles/PMC9333262/) |
| (observational health data) | 2023 | J. Big Data | random OS + US (8 strategies; imbalance ratios 20/10/2/1) across 58 tasks × 4 health databases | lasso, **RF, XGBoost** | **YES** (original-data baseline) | **AUROC**, max F1 | n/a (AUROC) | spread across 58 tasks | **Neither OS nor US improved AUROC** (lasso/XGBoost moved < 0.06); RF + oversampling was actively *harmful* (AUROC down ~0.3, worse with fewer events). The large-scale agreement with us. | [10.1186/s40537-023-00857-7](https://link.springer.com/article/10.1186/s40537-023-00857-7) (authors to confirm) |
...
*The modern critiques supply the missing control — and the result is ours.* van den Goorbergh et al. (2022, JAMIA) and Piccininni et al. (2024, J Biomed Inform) both compare against an **uncorrected baseline** and find that random undersampling (and NearMiss) **do not improve AUROC** and **severely harm calibration**, making models that strongly overestimate the minority probability. That miscalibration is a **threshold-independent harm invisible to F1/accuracy at 0.5** — and it is the same effect we see in our own speed experiment, where naive undersampling overpredicts until the prior-restoring weights are added. A large-scale study makes the discrimination point at scale: the observational-health-data study (J Big Data 2023) built **1,566 models across 58 tasks** with lasso, random forest and XGBoost and found **neither random oversampling nor undersampling improved AUROC**, with random-forest oversampling actively degrading it.

*Dispersion is the third axis, and it tracks the conclusion.* Across the empirical comparison studies, the result follows whether uncertainty is shown. The studies that report a resampling **win give point estimates with no dispersion** — Gurcan & Soylu (2024) and the class-weight medical study report ROC-AUC with no standard deviation. The studies that **account for uncertainty** reach the opposite conclusion: Hasanin et al. (2019) find undersampling and oversampling do not help once the uncertainty is shown (their first dataset), and Wainer (2018), on the threshold-independent metrics, finds resampling and the baseline tie. Kim & Hwang (2022) go furthest and test it statistically with a 5×2cv paired t-test across 31 datasets and eight classifiers: sampling significantly changed AUROC or AUPRC in only ~10–12% of cases, and more often reduced than improved performance. So a reported "win" is often an artifact of comparing point estimates, the same way it is an artifact of the default threshold — and these studies use **strong learners (RF, XGBoost)**, so it is not explained away by weak model choice.
...
- **~22 papers** here. The foundational/critique rows came from an adversarially-verified search; the empirical comparison studies were added from Sole's review (her reading). The Informatics in Medicine Unlocked and the J Big Data 2023 (observational health data) rows still need their exact author citations confirmed.
