"""
Train classifiers with AllKNN undersampling on various imbalanced datasets.
...
from configs.undersamplers import allknn
...
OUTPUT_DIR = REPO_ROOT / "models" / "allknn"
...
for dataset in tqdm(DATASETS_LS, desc="Datasets"):
    ...
    xtrainu, ytrainu, xtest, ytest, Xu, yu, stats = undersample_data(
        allknn, X_train, y_train, scale=True
    )
    ...
    for estimator, params in tqdm(
        ...
        joblib.dump(search, OUTPUT_DIR / f"{dataset}_{estimator}_allknn.pkl")
