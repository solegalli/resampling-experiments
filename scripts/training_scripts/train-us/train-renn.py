"""
Train classifiers with Repeated Edited Nearest Neighbours undersampling on various imbalanced datasets.

Undersampling is applied once before hyperparameter search to avoid resampling
on every candidate. RENN is more aggressive than ENN as it repeats the editing
step until no more samples are removed. Models are saved to disk for later evaluation.
"""
...
from configs.undersamplers import renn
...
OUTPUT_DIR = REPO_ROOT / "models" / "renn"
...
for dataset in tqdm(DATASETS_LS, desc="Datasets"):
    ...
    xtrainu, ytrainu, xtest, ytest, Xu, yu, stats = undersample_data(
        renn, X_train, y_train, scale=True
    )
    ...
    for estimator, params in tqdm(
        ...
        joblib.dump(search, OUTPUT_DIR / f"{dataset}_{estimator}_renn.pkl")
