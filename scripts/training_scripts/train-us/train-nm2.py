"""
Train classifiers with NearMiss-2 undersampling on various imbalanced datasets.

Undersampling is applied once before hyperparameter search to avoid resampling
on every candidate. NearMiss-2 is very aggressive — it keeps only majority class
samples whose average distance to the 3 farthest minority samples is smallest.
...
from configs.undersamplers import nm2
...
OUTPUT_DIR = REPO_ROOT / "models" / "nm2"
...
for dataset in tqdm(DATASETS_LS, desc="Datasets"):
    ...
    xtrainu, ytrainu, xtest, ytest, Xu, yu, stats = undersample_data(
        nm2, X_train, y_train, scale=True
    )
    ...
    for estimator, params in tqdm(
        ...
        joblib.dump(search, OUTPUT_DIR / f"{dataset}_{estimator}_nm2.pkl")
