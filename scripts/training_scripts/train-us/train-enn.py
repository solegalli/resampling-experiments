"""
Train classifiers with Edited Nearest Neighbours undersampling on various imbalanced datasets.
...
from configs.undersamplers import enn
...
OUTPUT_DIR = REPO_ROOT / "models" / "enn"
...
for dataset in tqdm(DATASETS_LS, desc="Datasets"):
    ...
    xtrainu, ytrainu, xtest, ytest, Xu, yu, stats = undersample_data(
        enn, X_train, y_train, scale=True
    )
    ...
    for estimator, params in tqdm(
        ...
        joblib.dump(search, OUTPUT_DIR / f"{dataset}_{estimator}_enn.pkl")
