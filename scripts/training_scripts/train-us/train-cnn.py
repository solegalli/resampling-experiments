"""
Train classifiers with Condensed Nearest Neighbour undersampling on various imbalanced datasets.

Undersampling is applied once before hyperparameter search to avoid resampling
on every candidate. CNN is one of the slowest undersamplers, so this pre-computation
approach gives a significant speed-up during tuning. Models are saved to disk
...
from configs.undersamplers import cnn
...
OUTPUT_DIR = REPO_ROOT / "models" / "cnn"
...
for dataset in tqdm(DATASETS_LS, desc="Datasets"):
    ...
    xtrainu, ytrainu, xtest, ytest, Xu, yu, stats = undersample_data(
        cnn, X_train, y_train, scale=True
    )
    ...
    for estimator, params in tqdm(
        ...
        joblib.dump(search, OUTPUT_DIR / f"{dataset}_{estimator}_cnn.pkl")
