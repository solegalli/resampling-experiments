"""
Train classifiers with Tomek Links undersampling on various imbalanced datasets.
...
from configs.undersamplers import tomek
...
OUTPUT_DIR = REPO_ROOT / "models" / "tomek"
...
for dataset in tqdm(DATASETS_LS, desc="Datasets"):
    ...
    xtrainu, ytrainu, xtest, ytest, Xu, yu, stats = undersample_data(
        tomek, X_train, y_train, scale=True
    )
    ...
    for estimator, params in tqdm(
        ...
        joblib.dump(search, OUTPUT_DIR / f"{dataset}_{estimator}_tomek.pkl")
