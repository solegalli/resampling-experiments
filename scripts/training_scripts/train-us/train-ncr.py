"""
Train classifiers with Neighbourhood Cleaning Rule undersampling on various imbalanced datasets.
...
from configs.undersamplers import ncr
...
OUTPUT_DIR = REPO_ROOT / "models" / "ncr"
...
for dataset in tqdm(DATASETS_LS, desc="Datasets"):
    ...
    xtrainu, ytrainu, xtest, ytest, Xu, yu, stats = undersample_data(
        ncr, X_train, y_train, scale=True
    )
    ...
    for estimator, params in tqdm(
        ...
        joblib.dump(search, OUTPUT_DIR / f"{dataset}_{estimator}_ncr.pkl")
